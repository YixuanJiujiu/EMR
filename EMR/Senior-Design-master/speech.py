...
def PopMessage(field,message):
 pop = Tk()
 pop.title("Pop up")
 pop.geometry('600x300')
 label_instruction = Label(pop, text="click the term and select field").pack()
 comboxlist = Combobox(pop,textvariable="")
 comboxlist["values"]=("HPI","Family History","Allergy","Medical History")
 comboxlist.current(field)
 comboxlist.pack()
 label_text = Label(pop,text = message).pack()
 Button(pop, text="confirm", command = pop.destroy).pack()
 pop.mainloop()
...
if __name__ == '__main__':
	...
	#
	#
	p_doctor = threading.Thread(target = Record, args=("doctor",))
	p_patient = threading.Thread(target = Record, args=("patient",))
     #   p_ui = Process(target = PopMessage, args=(3,"patient",))
	...
	#PopMessage(3,"I am allergic to Peanut")
